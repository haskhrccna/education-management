...
### Live verification (coordinator, 2026-08-17 — both GREEN after fixes)

Both journeys now **PASS** end to end (iPhone 17 Pro Max / iOS 26.2), each against a fresh reseed, after two real app fixes surfaced during live verification:

- **Journey 9 — server fix (owner-approved):** the confirmed broadcast-never-persists bug above was fixed. `broadcastMessage()` (and the BullMQ worker) now call `notifyUser()` per recipient, persisting a `notification` row + emitting the socket event + best-effort push (commits `7fefb2c` + `e572d17`; `admin.service.test.ts` "persists a durable notification row for every recipient", suite 28/28). Journey 9's `notifications.row.2` assertion passes.
- **Journey 7 — client UX fix (found by this journey):** Journey 7 failed for three sessions at the post-submit "OK" alert. Root cause (decisively diagnosed via a screenshotted diagnostic flow + a manual sim tap — NOT the earlier 429/ts-rest guesses): `student/teacher-change.tsx`'s `ScrollView` used RN's default `keyboardShouldPersistTaps="never"`, so the Submit tap while the reason keyboard was still open was consumed to dismiss the keyboard and never reached `onPress` (a real "tap Submit twice" UX papercut). Fixed with `keyboardShouldPersistTaps="handled"` (commit `f75e1ff`; BUGLOG entry added). Journey 7 passes Part 1 submit→OK, Part 2 admin `assign-teacher` approval, and Part 3 the reassignment on Ahmad's `teacher-appointments` (row.0 + decided-row.0); DB `teacher_change_requests.status = APPROVED`.

## E2E Coverage — Journeys 12+13: plan create→view, ijazah issue→see (Plan 2, Task 6)

Two cross-role WRITE journeys, `journeys/12-plan-create-view.yaml` and `journeys/13-ijazah-issue-see.yaml`. Both consume only pre-existing testIDs from `teacher/plans.tsx`, `teacher/ijazahs.tsx`, `student/plans.tsx`, `student/ijazahs.tsx` (all added in Plan 2 Task 2 + student Task 7). Neither touches `covered-screens.json`.

### Journey 12: `teacher/plans` create → `student/plans` view
- **Determinism:** on a fresh seed Ahmad (`teacher@`) has exactly one ACCEPTED student (Ali), so `teacher-plans.student.0` is Ali; `teacher/plans.tsx`'s `fetchMeta` also pre-selects `unique[0]`. No `CurriculumPlan` is seeded (`seed-e2e.ts` creates none), so `teacher-plans.row.0` (after submit) and `student-plans.row.0` (for Ali) both prove THIS create reached the student — not a pre-seeded artifact.
- **App fix (same class as Journey 7):** `teacher/plans.tsx`'s create-form `ScrollView` now sets `keyboardShouldPersistTaps="handled"` — the flow types `name-input` + `target-date-input` before tapping `add-item`/`submit`, so the first tap would otherwise be swallowed by keyboard dismissal.
- **Success proof:** native `Alert(t('success'), 'تم إنشاء الخطة')` (single default OK) asserted, then `teacher-plans.row.0`.

### Journey 13: `teacher/ijazahs` issue (SURAH) → `student/ijazahs` view
- **Determinism:** same single-ACCEPTED-student basis (`teacher-ijazahs.student.0` = Ali; student NOT pre-selected here, so the tap is required). Scope chips render `['SURAH','JUZ','FULL_QURAN']`, so `scope.0` = SURAH (also default). No ijazah seeded → `teacher-ijazahs.row.0` + `student-ijazahs.row.0` prove the issue.
- **Real business-rule precondition (found by this journey — NOT an app bug):** `ijazah.service.ts:34` correctly rejects a SURAH ijazah with 409 "not yet fully memorized" unless the student's `MemorizationProgress` for that surah is `COMPLETE`. The seed had NONE for Ali (`seed.ts`'s "MemorizationProgress for student1" comment actually inserts `revisionSchedule` rows, not memorization). Fixed by seeding Ali `COMPLETE` on Al-Fatiha (surah #1) in `seed-e2e.ts` — and because `getSurahs` orders by `number asc`, that surah IS the picker's `surah.0`, exactly what the flow selects. Safe against Journey 11 (which uses the separate `mushaf_page_status` table, page 3). Same `keyboardShouldPersistTaps="handled"` fix applied to `teacher/ijazahs.tsx` for consistency (SURAH scope focuses no text input, but the fix guards the JUZ/chain-ref paths).
- **Success proof:** native `Alert(t('success'), 'تم إصدار الإجازة')` asserted, then `teacher-ijazahs.row.0`.

### Live verification (coordinator, 2026-08-17 — both GREEN)
Both journeys **PASS** end to end individually against fresh reseeds (iPhone 17 Pro Max / iOS 26.2, commit `e003e98`). DB after each: 1 `curriculum_plans` / 1 `ijazahs`. `check-testids.js` OK (35 screens); mobile + server `tsc` clean.

## E2E Coverage — merged-feature smokes (biometric login, web build) — Plan 2 Task 7 addendum

Two features arrived via the mid-Plan-2 merge of `origin/main` and are covered here:

- **Biometric login (`expo-local-authentication`)** — `flows/auth/05-biometric-ui-smoke.yaml`, a deliberately UI-only smoke (Face ID / Touch ID unlock is a system sheet outside the app view hierarchy and not Maestro-drivable; the sim has no enrolled biometric). It asserts (1) the app boots to `login.screen` — i.e. the auth store's init-time `getBiometricStatus()` native-module probe did not crash the app (the exact crash the post-merge native rebuild fixed), and (2) the account screen's biometric settings toggle (`account.biometric-toggle`) renders. The conditional `login.biometric` button is intentionally NOT asserted (correctly absent on a fresh install with no enabled preference — asserting its visibility would be wrong). **Verified live: PASS.**
- **GitHub Pages web deployment** — verified `cd mobile && npm run build:web` (`expo export -p web && node scripts/prepare-github-pages.js`) compiles clean: exits 0, emits `dist/` (~8.2M: 3 JS bundles under `_expo/static/js/web/`, `index.html`, plus `404.html` + `.nojekyll` from the prepare script), no bundler errors. `dist/` is gitignored. Not an E2E flow (browser build check).
