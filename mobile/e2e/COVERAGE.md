...
- **Re-runnability**: `Grade` has no unique constraint (`packages/server/prisma/schema.prisma`), so unlike Journey 2's appointment booking, this journey has no duplicate-guard collision risk on repeated bare re-runs. `recordActivity`'s same-UTC-day dedupe rule means a same-day re-run won't push `currentStreak` past `"1"`, so the final assertion still holds on repeated same-day runs.
