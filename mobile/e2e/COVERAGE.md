...
- **What is deliberately NOT asserted, and why**: the streak's *numeric value* changing. A repo-wide grep confirms `markStudentActiveToday` (`gamification.service.ts` — the only function that ever writes to the `Streak` table) has no caller anywhere in the server's routes/controllers/services — grade submission, and every other student action, leaves `Streak` completely untouched. Asserting a value change would therefore be asserting behavior the app does not implement, not just a brittle content check. New BUGLOG.md finding this task ("the student streak is never updated by any real user action").
