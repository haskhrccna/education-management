...
| student-home | `student-home.quick-action.2` (grades) | `01-home-smoke.yaml` | tapped -> `student-grades.screen`, back via `bottom-nav.student-home`. **Fix-pass update:** full destination assertion restored — the wrong-destination finding (BUGLOG.md Finding #3) was root-caused to a Maestro `scrollUntilVisible` defect, not this tile; `.2` was never actually affected since it's above the fold and needs no scroll. |
| student-home | `student-home.quick-action.3` (reports) | `01-home-smoke.yaml` | tapped -> `student-reports.screen`, back via `reports.back` (no `BottomNav` on this screen). **Fix-pass update:** full destination assertion and return-tap restored, same root-cause resolution as `.2` above — `.3` is also above the fold and needs no scroll. |
...
- **Historical note, now resolved (Plan 1 findings fix-pass):** `student-home.quick-action.2/.3/.4` were previously believed to not reliably reach their intended destination (BUGLOG.md Finding #3). Root-caused to a Maestro `scrollUntilVisible` defect, not an app bug — `01-home-smoke.yaml` now taps all 7 quick-action tiles with full, strict destination assertions (see the per-control table above). `05-reports-smoke.yaml` and `06-revisions-smoke.yaml` still reach their screens via `openLink` deep links rather than the quick-action tap (written before this root cause was known) — that workaround is now known-unnecessary but restoring direct-tap coverage there was out of the fix-pass's scope; see BUGLOG.md's resolved Finding #3 for the follow-up note.
