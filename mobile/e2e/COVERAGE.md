...
### Journey 10: `student-gamification.streak` survives an unrelated grade write

- **What is asserted**: `student-gamification.streak` (the current/longest-streak `MetricTile` pair, a static container `View` regardless of value) is visible before AND after a teacher submits a fresh grade for ali, across a full relaunch + role-switch + relaunch round trip.
- **What is deliberately NOT asserted, and why**: the streak's *numeric value* changing after this journey's **grade** submission specifically. `recordActivity` (`gamification.service.ts` — the `Streak` table's real writer) has 6 genuine callers wiring it into other student actions (mushaf page-status, recording upload, revision completion, ijazah, memorization, curriculum-plan) — the streak genuinely does increment for those. `grade.service.ts` is confirmed (by direct grep, zero matches) not one of them, so a value-change assertion specifically after grading would assert behavior grading doesn't implement, even though other actions would legitimately support one. New BUGLOG.md finding this task, corrected post-review ("grade submission specifically does not count toward the student streak, unlike several other student actions which do").
- **Re-runnability**: `Grade` has no unique constraint (`packages/server/prisma/schema.prisma`), so unlike Journey 2's appointment booking, this journey has no duplicate-guard collision risk on repeated bare re-runs.
- **Live verification**: passed end to end on the **first** live attempt, no fixes needed. Cross-checked against Postgres: a new grade row (`'B'`/`ASSIGNMENT`) now exists for ali alongside the two seeded rows.
...
- **Re-runnability**: `PageMemorization` has `@@unique([userId, page])` and `setPageStatus` is an upsert, so — like Journey 10, unlike Journey 2 — this journey is safely re-runnable back-to-back without a DB reset.
...
Consolidated summary of every genuinely-uncovered interactive element across all 17 checker-enforced screens (student + auth) plus the journey-scoped admin/teacher testIDs, cross-checked against the per-screen tables above (each row already carries its own one-line justification — this section is the audit's roll-up, not a duplicate).
...
- **Modal/sheet backdrop-dismiss variants**: `student-appointments.date-modal-backdrop`/`.time-modal-backdrop`, `student-mushaf.status-modal-backdrop`, `student-mushaf.record-modal-backdrop` — the flows dismiss via an explicit close control instead; the backdrop tap itself is an untested but structurally-identical alternate path.
- **Real app bug prevents coverage**: `student-appointments.date-option.N`/`.time-option.N` — BUGLOG.md's date/time-picker accessibility-node-merging bug makes these individually unreachable by any Maestro selector, not a coverage choice.
- **Nondeterministic navigation prevents coverage**: `student-home.quick-action.2/.3/.4` don't reliably reach their intended destination (BUGLOG.md Finding #3, 3 of 7 tiles now confirmed affected) — all downstream screens they'd nominally reach are instead covered via deep link, and these three tiles are exercised only for tappability (`assertNotVisible: student-home.screen`), never destination.
...
**Nothing in this list is an unexplained gap** — every item above traces to one of: no seed data, error-only rendering, deliberate non-mutation, a documented app bug, or an explicit, brief-mandated scope boundary. `node mobile/scripts/check-testids.js` mechanically guarantees the orthogonal claim (every interactive element has *a* testID) for all 17 covered screens; this section covers the claim the checker can't make — that every testID is accounted for by a flow or a justified reason it isn't.
