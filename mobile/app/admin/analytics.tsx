  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.background }} edges={['top']}>
      <View style={[styles.header, { backgroundColor: COLORS.primary }]}>
        ...
        <AppText variant="headlineSmall" color="#FFFFFF">{t('analytics')}</AppText>
        <View style={{ width: 24 }} />
      </View>
        ...
        {error ? (
          ...
          <>
            ...
            {analytics.surahMissRates.length === 0 ? (
              <AppText variant="bodyMedium" color={COLORS.textSecondary}>{t('noData')}</AppText>
            ) : (
                      ...
                      <View
                        style={[
                          styles.bar,
                          {
                            ...
                            backgroundColor: item.missRate > 0.5 ? COLORS.error : item.missRate > 0.25 ? COLORS.warning : COLORS.success,
                          },
                        ]}
                      />
                    </View>
                    <AppText variant="bodySmall" color={COLORS.textMuted} style={{ marginTop: SPACING.xs, textAlign: isRTL ? 'left' : 'right' }}>
                  ...
                  <AppCard colors={COLORS} style={{ marginBottom: SPACING.sm }}>
                    <View style={styles.row}>
                      <View style={[styles.avatar, { backgroundColor: COLORS.primaryMuted }]}>
                        <AppText variant="bodyMedium" color={COLORS.primary}>{fullName(item.teacher).charAt(0)}</AppText>
                      </View>
                      <View style={{ flex: 1, marginStart: SPACING.md }}>
                        <AppText variant="titleSmall" color={COLORS.textPrimary}>{fullName(item.teacher)}</AppText>
                        <AppText variant="bodySmall" color={COLORS.textMuted}>{item.teacher.email}</AppText>
                        <View style={styles.teacherMetrics}>
                          <View style={styles.pill}>
                            <AppText variant="bodySmall" color={COLORS.textSecondary}>{t('activeStudents')}: {item.activeStudents}</AppText>
                          </View>
                          <View style={styles.pill}>
                            <AppText variant="bodySmall" color={COLORS.textSecondary}>{t('gradesLast30d')}: {item.gradesLast30d}</AppText>
                          </View>
                          <View style={styles.pill}>
                            <AppText variant="bodySmall" color={COLORS.textSecondary}>{t('sessionsLast30d')}: {item.sessionsLast30d}</AppText>
                          </View>
                        </View>
                      </View>
