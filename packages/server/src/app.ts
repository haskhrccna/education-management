...
import { withContentCache } from './middleware/cache.middleware';
...
app.use('/api/v1/surahs', authenticate, standardLimiter, withContentCache(86400), surahsRouter);
app.use('/api/v1/memorization', authenticate, standardLimiter, memorizationRouter);
// Mushaf page/surah payloads are static per user — 24h browser/SW cache + ETag.
app.use('/api/v1/mushaf/page', authenticate, standardLimiter, withContentCache(86400));
app.use('/api/v1/mushaf/surahs', authenticate, standardLimiter, withContentCache(86400));
